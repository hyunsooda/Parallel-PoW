...
//mpz_t numOfOperations;
const int targetBit = 6;
const int operationPerThread = 900;
...
int main(int argc, char* argv[]) {
	//mpz_init(&numOfOperations);
	//mpz_set_str(&numOfOperations, "104857599999999");
	const int numOfBlocks = 10; // 10000
	...
	dim3 dimGrid(1, 1, 1);
	dim3 dimBlock(1, 1, 1);
	myfunc<<<dimGrid, dimBlock>>> (d_nonce);
	CUDA_CHECK(cudaMemcpy(h_nonce, d_nonce, 1024 * sizeof(char), cudaMemcpyDeviceToHost));
	*/
	//printf("%s\n", h_nonce);
	
	/*
	char a1[200] = "1234";
	char a2[200] = "hellohellohellohellohellohellohello";
	char a3[200] = "byebyebyebyebyebye";
	char output[100] = { 0, };
	char bar[100] = { 0, };
	strcc(output, a1); strcc(output, a2); strcc(output, a3); printf("%s\n", output);
	sha256(output, mystrlen(output), bar);
	printf("%s\n%d %d\n", bar, mystrlen(bar), mystrlen(output));
	*/
	...
	/*
	foo<<<blocks, threads >>> (d_nonce);
	CUDA_CHECK(cudaPeekAtLastError());
	CUDA_CHECK(cudaMemcpy(h_nonce, d_nonce, sizeof(char) * 64, cudaMemcpyDeviceToHost));
	printf("%s\n", h_nonce);
	*/
	
	/*
	char* a = "qwkfqwkenwqken1kn1233123";
	char bar[100];
	for (int i = 0; i < 1000; i++) {
		sha256(a, mystrlen(a), bar);
		printf("%s\n", bar);
	}
	*/
	...
	char k3[100] = { 0, };
	char k4[100];
	strcc(k3, k1); strcc(k3, k2); 
	sha256(k3, mystrlen(k3), k4);
	printf("%s\n %d\n", k4, mystrlen(k4));
	*/
	/*
	char buf[30];
	myitoa(132456789, buf, 10);
	printf("%s\n", buf);
	*/
}
